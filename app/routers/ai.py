...
async def process_command(user_text: str, db: AsyncSession, user: User):
    ...
    # --- ГЛАВНЫЙ ПРОМПТ ---
    # Мы заставляем ИИ возвращать JSON
    system_prompt = (
        "Ты — голосовой ассистент банка BellyBank. "
        f"{finance_context}\n"
        "Твоя задача: определить намерение пользователя.\n"
        "1. Если пользователь хочет сделать ПЕРЕВОД, верни JSON:\n"
        "   {\"action\": \"transfer\", \"amount\": 500, \"phone\": \"8747...\", \"reply\": \"Хорошо, перевожу 500 тенге...\"}\n"
        "   Номер телефона должен быть чистым (только цифры).\n"
        "2. Если это просто вопрос, верни JSON:\n"
        "   {\"action\": null, \"reply\": \"Твой ответ...\"}\n"
        "Отвечай кратко и вежливо. Всегда возвращай только валидный JSON."
    )
